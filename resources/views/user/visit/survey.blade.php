@extends('user.master')
@section('title', trans_message('visit_form'))
@push('navbar')
    <div class="pt-6 flex lg:hidden">
        <span class="text-2xl font-semibold text-gray-800">{{ trans_message('visit_form') }}</span>
    </div>
@endpush
@section('style')
    <style>
    ...
    <section class="pt-4 sm:pt-6">
        ...
        <div
                            ...
                            foreach($category['questions'] as $question) {
                                ...
                                if(isset($old_questions[$question['categoryId']])) {
                                    ...
                                    if(!empty($report_result)) {
                                        $report_result = (array)$report_result[0];
                                        // Check if question is answered (not N/A and has an answer)
                                        if(isset($report_result['answer']) && $report_result['answer'] !== '' && $report_result['answer'] !== null) {
                                            $answeredQuestions++;
                                        } elseif(isset($report_result['na']) && $report_result['na'] == '1') {
            ...
            <div class="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
                <!-- Total Questions -->
                <div class="bg-white/80 backdrop-blur-sm rounded-lg p-4 border border-blue-100 shadow-sm">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs text-gray-500 font-medium">Total Questions</p>
                            <p class="text-2xl font-bold text-gray-800 mt-1">{{ $totalQuestions }}</p>
                        </div>
                ...
                <!-- Answered Questions -->
                <div class="bg-white/80 backdrop-blur-sm rounded-lg p-4 border border-green-100 shadow-sm">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs text-gray-500 font-medium">Answered</p>
                            <p class="text-2xl font-bold text-green-600 mt-1">{{ $answeredQuestions }}</p>
                        </div>
                ...
                <!-- Pending Questions -->
                <div class="bg-white/80 backdrop-blur-sm rounded-lg p-4 border border-orange-100 shadow-sm">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs text-gray-500 font-medium">Pending</p>
                            <p class="text-2xl font-bold text-orange-600 mt-1">{{ $pendingQuestions }}</p>
                        </div>
            ...
            <!-- Progress Bar -->
            <div class="mt-4 bg-white/80 backdrop-blur-sm rounded-lg p-4 border border-blue-100 shadow-sm">
        ...
        <div class="mb-6 overflow-x-auto whitespace-nowrap pb-2 scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0" id="category-navigation">
            <div class="flex gap-2">
                @if($payload['categories'] && count($payload['categories']) > 0)
                    @foreach($payload['categories'] as $cat_k => $category)
                        <button type="button"
                                onclick="jumpToView({{ $cat_k }})"
                                id="nav-btn-{{ $cat_k }}"
                                class="nav-category-btn px-4 py-2 rounded-full text-sm font-medium transition-colors border whitespace-nowrap flex items-center gap-2
                                {{ $cat_k === 0 ? 'bg-[#0073AF] text-white border-[#0073AF]' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50' }}">
                            @if(isset($category['imageUrl']) && $category['imageUrl'])
                                <img src="{{ $category['imageUrl'] }}" alt="" class="w-4 h-4 object-contain">
                            @endif
                            {{ $category['name'] }}
                        </button>
                    @endforeach
                            ...
                            <div class="text-center mb-6 relative">
                                <div class="flex items-center justify-center gap-2 mb-4">
                                    ...
                                    @php
                                        ...
                                        $categoryTotalQuestions = isset($category['questions']) ? count($category['questions']) : 0;
                                        $categoryAnsweredQuestions = 0;
                                        ...
                                        $categoryPendingQuestions = $categoryTotalQuestions - $categoryAnsweredQuestions;
                                    @endphp
                                    
                                    <div class="flex flex-col items-center gap-2">
                                        <span class="flex items-center gap-2 text-2xl font-semibold text-gray-800">
                                        ...
                                        <div class="flex items-center gap-2 text-sm">
                                            <span class="px-3 py-1 rounded-full {{ $categoryPendingQuestions > 0 ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700' }} font-medium">
                                                @if($categoryPendingQuestions > 0)
                                                    {{ $categoryPendingQuestions }} {{ $categoryPendingQuestions == 1 ? 'question' : 'questions' }} remaining
                                                @else
                                                    All {{ $categoryTotalQuestions }} questions completed ✓
                                                @endif
                                            </span>
                                            <span class="text-gray-500 font-medium">
                                                {{ $categoryAnsweredQuestions }}/{{ $categoryTotalQuestions }}
                                            </span>
                                        </div>
                                    </div>
                                    
                                    <div class="w-6 h-6 {{ $categoryPendingQuestions > 0 ? 'bg-orange-500' : 'bg-green-500' }} rounded-full flex items-center justify-center">
                                    ...
                                    @foreach($category['questions'] as $question)
                                        ...
                                        <div class="border border-gray-300 md:px-30 lg:px-20 xl:px-60 p-[20px] rounded-lg text-center w-full question-container">
                                            ...
                                            @if ($question['type']==='RADIO' || $question['type']==='SELECT')
                                                <div class="w-full mt-4 flex justify-center gap-1 sm:gap-3">
                                                    @if($allOptionTexts)
                                                        @foreach($allOptionTexts as $optKey => $optVal)
                                                            <div class="flex flex-col items-center gap-2 cursor-pointer">
                                                                ...
                                                                <label for="opt-{{ $question['id'] }}-{{ $optKey }}" class="cursor-pointer transition-transform duration-200 peer-checked:scale-110 peer-checked:ring-4 peer-checked:ring-blue-500 rounded-full">
                                                                    <img src="{{ asset('assets/emojis/glossy/'.($optKey==0?'00.png':'01.png')) }}" alt="{{ $optVal }}" class="w-12 h-12">
                                                                </label>
                                                                <span class="hidden text-sm font-medium text-gray-700">{{ $optVal??'' }}</span>
                                                            </div>
                                                        @endforeach
                                                    @endif
                                                </div>
                                            ...
                                            @if ($question['type']==='SLIDER')
                                                ...
                                                @endphp
                                                <div class="mt-4">
                                                    <div class="flex justify-center items-center gap-1 sm:gap-3 flex-wrap">
                                                        @foreach($allOptionTexts as $index => $emoji)
                                                            @php
                                                                $score = $allOptionScores[$index];
                                                                $isSelected = ($score == $defaultValue);
                                                                $glossyImages = ['00.png', '04.png', '03.png', '02.png', '01.png']; // Red, Orange, Yellow, Light Green, Dark Green
                                                                $img = $glossyImages[$index] ?? '00.png';
                                                            @endphp
                                                            <div class="flex flex-col items-center">
                                                                ...
                                                                <label for="slider-opt-{{ $question['id'] }}-{{ $index }}"
                                                                       class="cursor-pointer transition-transform duration-200 peer-checked:scale-110 peer-checked:ring-4 peer-checked:ring-blue-500 rounded-full hover:scale-105">
                                                                    <img src="{{ asset('assets/emojis/glossy/'.$img) }}" alt="{{ $emoji }}" class="w-12 h-12">
                                                                </label>
                                                                <span class="hidden text-xs mt-1 text-gray-600">{{ $emoji }}</span>
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                </div>
                                            ...
                                            @if ($question['type']==='AMOUNT')
                                                ...
                                                <div class="image-container mt-3">
                                                    <div class="image-preview border border-gray-300 my-2 p-3 rounded-[8px] md:h-[310px] relative {{ !isset($report_result['answer_image']) || empty($report_result['answer_image']) ? 'hidden' : '' }}">
                                                        <img src="{{ isset($report_result['answer_image'])?$report_result['answer_image']:'' }}" alt="Preview" class="h-[100%] w-full object-cover">
                                                        <button type="button" class="absolute bg-red-500 cursor-pointer h-5 remove-image-btn right-1 rounded-full text-white top-1 w-5" data-image="{{ isset($report_result['answer_image'])?$report_result['answer_image']:'' }}">&times;</button>
                                                    </div>
                                                    <label class="flex items-center gap-4 mb-3 cursor-pointer image-input justify-center {{ !isset($report_result['answer_image']) || empty($report_result['answer_image']) ?'':'hidden' }}">
                                                        ...
                                                        <input type="file" accept="image/*" name="question[{{ $question['id'] }}][answer_image]" class="hidden question_image">
                                                    </label>
                                                    <input type="hidden" name="question[{{ $question['id'] }}][answer_old_image]" value="{{ !isset($report_result['answer_image']) || empty($report_result['answer_image']) ?'':$report_result['answer_image'] }}" class="hidden answer_old_image">
                                                </div>
                                            @endif
                                            ...
                                            <div class="modern-card">
                                                <input type="hidden" name="question[{{ $question['id'] }}][comment_type]" class="input_comment" value="{{ (isset($report_result['comment_type']))?$report_result['comment_type']:'' }}">
                                                    ...
                                                    <div class="flex items-center gap-3">
                                                        ...
                                                        <div class="custom-checkbox-container text-xs !text-gray-700 !bg-transparent">
                                                            <label class="custom-checkbox-label flex items-center gap-2 cursor-pointer hover:bg-gray-50 rounded-md px-2 py-1 transition-colors duration-200">
                                                                <input type="checkbox" name="question[{{ $question['id'] }}][na]" value="1" id="checkbox" {{ (isset($report_result['na']) && $report_result['na']==1)?'checked':'' }}
                                                                class="custom-checkbox na-checkbox" data-type="{{ $question['type'] }}" data-id="{{ $question['id'] }}">
                                                ...
                                                <div class="tab-content">
                                                    ...
                                                    <div id="tab-comments-{{ $question['id'] }}" class="tab-pane {{ (isset($report_result['comment_type'])) && $report_result['comment_type']=='comment'?'':'hidden' }}">
                                                        <div class="pt-4 sm:pt-6">
                                                            <div class="comment-section bg-white">
                                                                <div class="pb-4">
                                                                    <h3 class="text-lg font-semibold text-gray-800 mb-2">
                                                                        {{ trans_message('add_comment') }}
                                                                    </h3>
                                                                    <p class="text-sm text-gray-500">Share your thoughts or feedback</p>
                                                                </div>

                                                                <div class="comment-form flex flex-col">
                                                                    <textarea class="comment-textarea w-full p-4 border border-gray-300 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                                                                              name="question[{{ $question['id'] }}][comment]" rows="6" placeholder="Add Comment Here..."
                                                                              id="commentText">{{ isset($report_result['comment'])?$report_result['comment']:'' }}</textarea>

                                                                    <div class="flex justify-end gap-3 mt-4">
                                                                        <button type="button"
                                                                                class="px-6 py-3 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-gray-300 commentBtn" data-type="">
                                                    ...
                                                    <div id="tab-photo-{{ $question['id'] }}" class="tab-pane {{ (isset($report_result['comment_type'])) && $report_result['comment_type']=='photo'?'':'hidden' }}">
                                                        <div class="pt-4 sm:pt-6">
                                                            <div class="photo-upload-section">
                                                                <div class="mb-4">
                                                                    <h3 class="text-lg font-semibold text-gray-800 mb-2">
                                                                        {{ trans_message('upload_photo') }}
                                                                    </h3>
                                                                    <p class="text-sm text-gray-500"> {{ trans_message('select_drag_image') }}</p>
                                                                </div>
                                                                ...
                                                                <div class="photo-upload-area flex flex-col">
                                                                    ...
                                                                    <label class="flex items-center gap-4 mb-3 cursor-pointer image-input justify-center">
                                                                        <div class="size-[100px] bg-gray-100 border flex items-center justify-center border-dashed border-gray-300 rounded-lg overflow-hidden relative">
                                                                            ...
                                                                            <img class="absolute inset-0 w-full h-full object-cover hidden" alt="preview">
                                                                        </div>
                                                                    ...
                                                                    <!-- Preview container -->
                                                                    <div id="previewContainer" class="previewContainer mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
                                                                        @if($old_photos)
                                                                            @foreach($old_photos as $oldImg)
                                                                                <div class="relative group">
                                                                                    <input type="hidden" name="question[{{ $question['id'] }}][old_photo][]" value="{{ $oldImg }}">
                                                                                    <img src="{{ asset(SURVEY_QUESTIONNAIRE_IMAGES_PATH.$oldImg) }}" class="w-full h-32 object-cover rounded-lg shadow-sm border">
                                                                                    <button type="button" data-image="{{ $oldImg }}"
                                                ...
                                                <div class="relative">
                                                    <div class="flex items-center gap-3 p-4 border border-gray-300 rounded-lg focus-within:border-blue-500 focus-within:bg-white transition-all duration-200">
                                                        ...
                                                        <input type="text" name="started_date" placeholder="Started"
                                                               ...
                                                               value="{{ (isset($visit_report_data->started_date)?$visit_report_data->started_date:'') }}" id="startedDateTime">
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="relative">
                                                    <div class="flex items-center gap-3 p-4 border border-gray-300 rounded-lg focus-within:border-green-500 focus-within:bg-white transition-all duration-200">
                                                        ...
                                                        <input type="text" name="completed_date" placeholder="Completed"
                                                               class="last_date flex-1 bg-transparent text-gray-700 font-medium focus:outline-none"
                                                               value="{{ (isset($visit_report_data->completed_date)?$visit_report_data->completed_date:'') }}" id="completedDateTime">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div class="flex mt-2 {{ $cat_k==0?'justify-end':'justify-between' }}">
                                    ...
                                    @else
                                        <div>
                                            <button type="button"
                                                    class="cursor-pointer bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded-lg font-medium transition-colors">
                                                {{ trans_message('save_for_later') }}
                                            </button>
                                            <button type="button" data-visit_id="{{ $visit->id }}" data-user_id="{{ $visit->visitor_id??'' }}"
                                                ...
                                                {{ trans_message('submit_questions') }}
                                            </button>
                                        </div>
                                    @endif
                                </div>
...
@section('script')
    <script>
        ...
        function updateNavigationState(index) {
            document.querySelectorAll('.nav-category-btn').forEach((btn, i) => {
                if (i === index) {
                    btn.classList.remove('bg-white', 'text-gray-600', 'border-gray-200', 'hover:bg-gray-50');
                    btn.classList.add('bg-[#0073AF]', 'text-white', 'border-[#0073AF]');
                } else {
                    btn.classList.add('bg-white', 'text-gray-600', 'border-gray-200', 'hover:bg-gray-50');
                    btn.classList.remove('bg-[#0073AF]', 'text-white', 'border-[#0073AF]');
                }
            });
        ...
        setTimeout(() => {
            ...
            // Emoji radio button click handler for SLIDER type
            $('.emoji-radio-input').on('change', function() {
        ...
        $(document).delegate('.na-checkbox', 'click', function (e) {
            ...
            if (el.prop('checked')) {
                if(type === 'RADIO' || type === 'SELECT'){
                    ...
                    // Remove visual selection from all emoji buttons and add disabled state
                    container.find('.emoji-button')
                        .removeClass('bg-blue-100 ring-4 ring-blue-500 scale-110')
                        .addClass('bg-gray-100 opacity-50 cursor-not-allowed');

                    // Reset and hide score display
                    container.find(`.slider-score-display-${id}`).text('Score: -').addClass('invisible');
                }else{
                ...
                if(type === 'RADIO' || type === 'SELECT'){
                    ...
                    container.find('input[name="question['+id+'][answer]"]').removeAttr('disabled');
                    // Remove disabled state from emoji buttons
                    container.find('.emoji-button').removeClass('opacity-50 cursor-not-allowed');
                    // Show score display
                    container.find(`.slider-score-display-${id}`).removeClass('invisible');
                }else{
        ...
        $(document).delegate('.remove-image-btn', 'click', function (e) {
            ...
            $.ajax({
                url: '{{ route('visit.response.delete.comment.image') }}',
                type: 'POST',
                data: {
        ...
        function autoSaveForm(callback = null, immediate = false) {
            ...
            if (immediate) {
                ...
                autoSaveTimer = setTimeout(saveFunc, 1000); // wait 1 second after last change
            }
        }
        ...
        function previewImages(visit_id, id, cat_id, input){
            ...
            setTimeout(function () {
                $.ajax({
                    url: '{{ route('visit.response.comment.image') }}',
                    method: 'POST',
                    data: {
                        _token: $('meta[name="csrf-token"]').attr('content'),
                        visit_id:visit_id,
        ...
        $(document).on("click", ".questionnaire_submit", function(e) {
            ...
            var visit_id = btn.attr('data-visit_id');
            var user_id = btn.attr('data-user_id');

            // Validate all required questions across all categories
            const incompleteCategories = validateAllCategories();
            
            if (incompleteCategories.length > 0) {
                // Show sidebar with incomplete categories
                showIncompleteCategoriesSidebar(incompleteCategories);
                return;
            }
            ...
            // Force immediate save first to ensure dates are saved
            autoSaveForm(function() {
                // After save is successful, submit the questionnaire
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        setTimeout(function () {
                            window.location.href = "{{route('visit.index',['page'=>'pending'])}}";
                        }, 1000);
                    },
                    error: function (xhr) {
            ...
            }, true); // Pass true for immediate execution
        });

        /**
         * Validate all required questions across all categories
         * Returns an array of incomplete categories with their details
         */
        function validateAllCategories() {
            const incompleteCategories = [];
            
            // Loop through all category views
            $('.view-content').each(function(index) {
                const categoryView = $(this);
                const categoryName = categoryView.find('.text-2xl.font-semibold').text().trim();
                const categoryId = categoryView.attr('id');
                const unansweredQuestions = [];
                
                // Find all question containers in this category
                categoryView.find('.question-container').each(function() {
                    const questionContainer = $(this);
                    const questionTitle = questionContainer.find('.font-semibold.text-black').text().trim();
                    
                    // Check if N/A checkbox is checked
                    const naCheckbox = questionContainer.find('.na-checkbox');
                    const isNA = naCheckbox.is(':checked');
                    ...
                    const questionType = naCheckbox.attr('data-type');
                    let isAnswered = false;
                    
                    // Check based on question type
                    if (questionType === 'RADIO' || questionType === 'SLIDER' || questionType === 'SELECT') {
                        // Check if any radio button is selected
                        const radioInputs = questionContainer.find('input[type="radio"]');
                        isAnswered = radioInputs.filter(':checked').length > 0;
                    } else if (questionType === 'AMOUNT') {
                        // Check if amount input has a value
                        const amountInput = questionContainer.find('input[type="number"]');
                        isAnswered = amountInput.val() && amountInput.val().trim() !== '';
                    } else if (questionType === 'TEXT') {
                        // Check if text input has a value
                        const textInput = questionContainer.find('input[type="text"]');
                        isAnswered = textInput.val() && textInput.val().trim() !== '';
                    } else if (questionType === 'COMMENT') {
                        // Check if textarea has a value
                        const textarea = questionContainer.find('textarea');
                        isAnswered = textarea.val() && textarea.val().trim() !== '';
                    }
                ...
                // If there are unanswered questions in this category, add to incomplete list
         ...
         * Show sidebar with incomplete categories
         */
        function showIncompleteCategoriesSidebar(incompleteCategories) {
            // Remove existing sidebar if any
            $('#incompleteCategoriesSidebar').remove();
            
            // Build categories list HTML
            let categoriesHtml = '';
            incompleteCategories.forEach(function(category, idx) {
                // Determine plural form for questions
                const questionPlural = category.unansweredCount === 1 ? '{{ trans_message("question_singular") }}' : '{{ trans_message("question_plural") }}';
                const questionsText = `${category.unansweredCount} {{ trans_message("questions_remaining", ["count" => ""]) }}`.replace(':count', category.unansweredCount).replace(':plural', questionPlural);
                
                categoriesHtml += `
                    <div class="category-item p-4 border-b border-gray-200 hover:bg-gray-50 cursor-pointer transition-colors duration-200"
                         onclick="navigateToCategory(${category.index})">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3 flex-1">
                                <span class="text-gray-600 font-medium text-sm">${idx + 1}.</span>
                                <div class="flex-1">
                                    <h5 class="font-medium text-gray-800">${category.name}</h5>
                                    <p class="text-xs text-gray-500 mt-0.5">${questionsText}</p>
                                </div>
                            </div>
                            <span class="bg-[#0073AF] text-white text-xs font-medium px-2.5 py-1 rounded">
                                ${category.unansweredCount}
                            </span>
                        </div>
                    </div>
                `;
            ...
            // Determine plural forms for header
            const categoryWord = incompleteCategories.length === 1 ? '{{ trans_message("category_singular") }}' : '{{ trans_message("category_plural") }}';
            const needsWord = incompleteCategories.length === 1 ? '{{ trans_message("needs_singular") }}' : '{{ trans_message("needs_plural") }}';
            const attentionText = `${incompleteCategories.length} ${categoryWord} ${needsWord} {{ trans_message("categories_need_attention", ["count" => "", "word" => "", "plural" => ""]) }}`.replace(':count', '').replace(':word', '').replace(':plural', '').replace('  ', ' ').trim();
            
            const sidebarHtml = `
                <div id="incompleteCategoriesSidebar" class="fixed inset-0 z-50 flex justify-end">
                    <!-- Overlay -->
                    <div class="absolute inset-0 bg-opacity-50 animate-fadeIn" onclick="closeSidebar()"></div>
                    
                    <!-- Sidebar -->
                    <div class="relative bg-white w-full max-w-md h-full shadow-2xl animate-slideInRight flex flex-col">
                        <!-- Header -->
                        <div class="flex items-center justify-between p-6 border-b border-gray-200">
                            <div>
                                <h3 class="text-lg font-semibold text-gray-800">{{ trans_message('incomplete_categories') }}</h3>
                                <p class="text-sm text-gray-500 mt-1">${attentionText}</p>
                            </div>
                            <button onclick="closeSidebar()" class="text-gray-400 hover:text-gray-600 transition-colors">
        ...
        function navigateToCategory(categoryIndex) {
            ...
            views.forEach((view, index) => {
                view.classList.toggle('hidden', index !== currentView);
            });
            ...
            updateNavigationState(categoryIndex);
            
            // Show toast
            sendToast('Please complete all required questions in this category', 'info');
        }
        ...
        function closeSidebar() {
            const sidebar = $('#incompleteCategoriesSidebar');
            const sidebarContent = sidebar.find('.relative.bg-white');
            
            // Add slide-out animation
            sidebarContent.css('animation', 'slideOutRight 0.3s ease-out');
            sidebar.find('.absolute.inset-0').fadeOut(300);
        ...
        /**
         * Update question counts in real-time
         */
        function updateQuestionCounts() {
            ...
            $('.view-content').each(function(categoryIndex) {
                const categoryView = $(this);
                let categoryTotal = 0;
                ...
                const categoryPending = categoryTotal - categoryAnswered;
                const categoryHeader = categoryView.find('.flex.flex-col.items-center.gap-2');
                
                if (categoryHeader.length > 0) {
                    // Update the badge text
                    const badge = categoryHeader.find('.px-3.py-1.rounded-full');
                    const counter = categoryHeader.find('.text-gray-500.font-medium');
                    const statusIcon = categoryView.find('.w-6.h-6.rounded-full');
                    
                    if (categoryPending > 0) {
                        // Update badge - orange for pending
                        badge.removeClass('bg-green-100 text-green-700').addClass('bg-orange-100 text-orange-700');
                        badge.text(`${categoryPending} ${categoryPending == 1 ? 'question' : 'questions'} remaining`);
                        
                        // Update icon - orange warning
                        statusIcon.removeClass('bg-green-500').addClass('bg-orange-500');
                        statusIcon.find('svg').html('<path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"></path>');
                    } else {
                        // Update badge - green for complete
                        badge.removeClass('bg-orange-100 text-orange-700').addClass('bg-green-100 text-green-700');
                        badge.html(`All ${categoryTotal} questions completed ✓`);
                        
                        // Update icon - green checkmark
                        statusIcon.removeClass('bg-orange-500').addClass('bg-green-500');
                        statusIcon.find('svg').html('<path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>');
                    }
                    
                    // Update counter (X/Y)
                    counter.text(`${categoryAnswered}/${categoryTotal}`);
                }
            });
            ...
            $('.bg-white\\/80.backdrop-blur-sm.rounded-lg.p-4.border.border-blue-100 .text-2xl.font-bold.text-gray-800').text(totalQuestions);
            $('.bg-white\\/80.backdrop-blur-sm.rounded-lg.p-4.border.border-green-100 .text-2xl.font-bold.text-green-600').text(answeredQuestions);
            $('.bg-white\\/80.backdrop-blur-sm.rounded-lg.p-4.border.border-orange-100 .text-2xl.font-bold.text-orange-600').text(pendingQuestions);
            
            // Update progress bar
            $('.bg-gradient-to-r.from-blue-500.to-blue-600').css('width', `${progressPercentage}%`);
            $('.text-sm.font-bold.text-blue-600').text(`${progressPercentage}%`);
        }
        
        // Attach event listeners to update counts when questions are answered
        $(document).ready(function() {
            // Listen for radio button changes (RADIO, SLIDER, SELECT)
            $(document).on('change', 'input[type="radio"]', function() {
                setTimeout(updateQuestionCounts, 100);
            });
            
            // Listen for text input changes (TEXT, AMOUNT)
            $(document).on('blur', 'input[type="text"], input[type="number"]', function() {
                setTimeout(updateQuestionCounts, 100);
            });
            
            // Listen for textarea changes (COMMENT)
            $(document).on('blur', 'textarea', function() {
                setTimeout(updateQuestionCounts, 100);
            });
            
            // Listen for N/A checkbox changes
            $(document).on('change', '.na-checkbox', function() {
                setTimeout(updateQuestionCounts, 100);
            });
