...
<head>
    <meta charset="utf-8"/>
    <title>Sign In | MysteryVisits </title>
    ...
    <div class="flex justify-center items-center w-full ">
        <div class="w-full max-w-md">
            <div class="bg-white shadow-lg rounded-2xl p-4 md:p-8">
                ...
                <form class="flex flex-col gap-6" id="loginForm" method="POST">
                    @csrf
                    <div>
                        ...
                        <p class='text-red-600 text-sm' id="email-error"></p>
                    </div>

                    <!-- Old password (without eye button) -->
{{--                    <div>--}}
{{--                        <div class="flex items-center justify-between">--}}
{{--                            <label class="text-xs sm:text-sm font-medium text-gray-700"--}}
{{--                                   for="password">Password</label>--}}
{{--                            <a class="text-sm text-blue-600 hover:underline" href="{{route('company.forget.password.get')}}">Forgot password?</a>--}}
{{--                        </div>--}}
{{--                        <input autocomplete="current-password"--}}
{{--                               class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-sm"--}}
{{--                               id="password" name="password" type="password">--}}
{{--                        <i class="ri-eye-line password-toggle-pwd position-absolute"></i>--}}
{{--                        <p class='text-red-600 text-sm' id="password-error"></p>--}}
{{--                    </div>--}}

                    <div class="relative">
                        <div class="flex items-center justify-between mb-1">
                            <label class="text-xs sm:text-sm font-medium text-gray-700" for="password">Password</label>
                            <a class="text-sm text-blue-600 hover:underline" href="{{ route('company.forget.password.get') }}">
                                Forgot password?
                            </a>
                        </div>
                        <input autocomplete="current-password" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-sm pr-10" id="password" name="password" type="password" placeholder="Enter your password">
                        <i id="togglePassword" class="ri-eye-line password-toggle-pwd absolute right-3 top-8 cursor-pointer text-gray-500"></i>
                        <p class="text-red-600 text-sm mt-1" id="password-error"></p>
                    </div>

                    <div>
                        <button
                                class="w-full py-2.5 text-xs sm:text-sm rounded-md btn-primary transition-all duration-300"
                                type="submit" id="loginButton">
                            <i class="bx bx-loader spinner me-2"
                               style="display: none" id="loginBtnSpinner"></i>Sign In
                        </button>
                    </div>
                </form>
                {{--
...
<script !src="">
    $(document).ready(function () {
        ...
        $(document).on('click', '.password-toggle-pwd', function() {
