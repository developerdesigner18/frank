    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">Pending Visit</h4>

                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="javascript: void(0);">Pending Visit</a></li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <script !src="">
        $(document).ready(function () {
            ...
            var dataTable = $('#visitDT').DataTable({
                ...
                ajax: {
                    url: '{{ route("admin.visit.pending.list") }}',
                    type: "POST",
                    dataType: "JSON",
                    data: function (f) {
                        f._token = "{{csrf_token()}}";
            ...
            // Request visit
            function approvedVisit(id, element) {
                Swal.fire({
                    title: "Are you sure?",
                    text: "Are you sure you want to Approved this visit?",
                    icon: "warning",
                    showCancelButton: !0,
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{route('admin.visit.addScore')}}",
                            dataType: "JSON",
                            method: "POST",
                            data: {
                                "id": id,
            ...
            /*function approvedVisit(id, element) {
                Swal.fire({
                    title: "Approved this visit",
                    html: `
                        <input type="number" step="any" id="swal-total-score" class="swal2-input mx-0 w-100" placeholder="Total Score..." value="">
                        <textarea id="swal-admin-notes" class="swal2-textarea mx-0 w-100" placeholder="Notes..."></textarea>
                    `,
                    focusConfirm: false,
                    ...
                    buttonsStyling: !1,
                    preConfirm: () => {
                        const total_score = document.getElementById("swal-total-score").value.trim();
                        const admin_notes = document.getElementById("swal-admin-notes").value.trim();

                        return new Promise((resolve, reject) => {
                            if (!total_score) {
                                reject("total score cannot be empty");
                            }
                            console.log(total_score);
                            if (!admin_notes) {
                                reject("admin notes cannot be empty");
                            }

                            resolve({ total_score, admin_notes });
                        });
                    }
                }).then(function (result) {
                    const { total_score, admin_notes } = result.value;
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{route('admin.visit.addScore')}}",
                            dataType: "JSON",
            ...
            window.approvedVisit = approvedVisit;
        });
    </script>
@endsection
