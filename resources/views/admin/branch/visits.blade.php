...
@section('title',$branch->branch_name.' Visits')
@push('navbar')
    <div class="flex items-center gap-1 pt-4">
        ...
        <span class="text-sm font-medium text-gray-500">/</span>
        <span class="text-sm font-medium text-gray-500">{{ $branch->branch_name }}</span>
    </div>
    <div class="pt-6 flex justify-between items-center lg:hidden">
        <span class="text-2xl font-semibold text-gray-800">{{ $branch->branch_name }} Visits</span>
    </div>
@endpush
@push('modal')
    <!-- edit Visit Model -->
...
@section('script')
    <script>
        ...
        function visitList() {
            ...
            $.ajax({
                url: "{{ route("admin.company.branch.visits.list",["branchId" => $branchId]) }}",
                dataType: "JSON",
                method: "POST",
                data: {
                    "sort_by": sort_by,
