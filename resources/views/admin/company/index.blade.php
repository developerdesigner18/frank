...
@push('navbar')
    <div class="pt-6 flex justify-between items-center lg:hidden">
        ...
        <div class="relative w-auto">
            <button command="show-modal" commandfor="add-company-model"
                ...
                <span>{{ trans_message('add_company') }}</span></button>
...
@push('modal')
    ...
    <!-- Add Company Model -->
    <el-dialog>
        <dialog id="add-company-model" aria-labelledby="dialog-title"
                        ...
                        <div class="sm:flex flex-col sm:items-start">
                            <div class="flex pb-4 justify-between w-full items-center">
                                <h3 id="add-visit-dialog-title" class="text-base font-semibold text-gray-800">
                                    {{ trans_message('company_profile_info') }}
                                </h3>
                                <button type="button" command="close" commandfor="add-company-model"
                            ...
                            <form class="w-full" id="addCompanyForm">
                                @csrf
                                ...
                                <div class="flex flex-col gap-1">
                                    ...
                                    <input type="text"
                                           ...
                                           placeholder="{{ trans_message('company_name') }}">
                                    <label id="company_name-error" class="text-[red] error" for="company_name"
    ...
    <!-- Edit Company Model -->
    <el-dialog>
        <dialog id="edit-company-model" aria-labelledby="dialog-title"
                class="fixed inset-0 size-auto max-h-none max-w-none overflow-y-auto bg-transparent backdrop:bg-transparent">
            <el-dialog-backdrop
                    class="fixed inset-0 bg-gray-900/50 transition-opacity data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in"></el-dialog-backdrop>
            ...
            <div tabindex="0"
                ...
                <el-dialog-panel
                    ...
                    <div class="bg-white relative px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex flex-col sm:items-start">
                            <div class="flex pb-4 justify-between w-full items-center">
                                <h3 id="edit-company-dialog-title" class="text-base font-semibold text-gray-800">
                                    {{ trans_message('company_profile_info') }}</h3>
                                <button type="button" command="close" commandfor="edit-company-model"
                                        class="rounded-full p-2 text-gray-800 hover:bg-gray-200 transition-all duration-300 cursor-pointer absolute right-3 top-3">
                                    <svg class="size-[18px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                         fill="currentColor">
                                        <path
                            ...
                            <form class="w-full" id="editCompanyForm">
                                @csrf
                                <input type="hidden" name="id" id="editId">
                                <!-- Company ID -->
                                <div class="flex gap-1 pb-4">
                                    <label class="text-sm font-medium text-gray-700">{{ trans_message('id') }} :</label>
                                    <span class="text-sm text-gray-600 flex w-[calc(100%-30px)] editCompanyId"></span>
                                </div>

                                <!-- Active Status Toggle -->
                                <div class="flex items-center justify-between pb-4">
                                    <label class="text-sm font-medium text-gray-700">{{ trans_message('active') }}</label>
                                ...
                                <div class="flex flex-col items-center">
                                    <div class="relative">
                                        ...
                                        <!-- File Input (hidden) -->
                                        <input type="file" id="edit-file-input" accept=".jpeg,.jpg,.png,.gif,.avif,.svg"
                                               class="hidden" name="image"/>
                                ...
                                <div class="flex flex-col gap-1 pb-4">
                                    <label class="block text-sm font-medium text-gray-700">{{ trans_message('name') }}</label>
                                    <input type="text" value="" id="editCompanyName"
                                           name="company_name"
                                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                           placeholder="{{ trans_message('company_name') }}">
                                    <label id="editCompanyName-error" class="text-[red] error" for="editCompanyName"
                                           style="display: none"></label>
                                </div>
                                <!-- Subdealer Selection -->
                                <div class="flex flex-col gap-1 pb-4">
               ...
               focus:border-transparent appearance-none"
                                ...
                                <!-- Account Admins Section -->
                                <div class="flex flex-col gap-1 pb-4">
                                    <h4 class="text-sm font-medium text-gray-700">{{ trans_message('account_admins') }}</h4>
                                    <div class="flex flex-col gap-3 company_admins_list">
                                    </div>
                                    <button type="button" command="show-modal" commandfor="invite-company-model"
                                            class="btn-primary px-4 py-2.5 text-sm font-medium text-white bg-blue-600 text-xs border border-blue-600 rounded-lg hover:bg-blue-700 cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-300 opacity-60">
                                        {{ trans_message('invite_company_admin') }}
                                    </button>
                                </div>
    ...
    <!-- Invite Company Admin Model -->
    <el-dialog>
        <dialog id="invite-company-model" aria-labelledby="dialog-title"
                class="fixed inset-0 size-auto max-h-none max-w-none overflow-y-auto bg-transparent backdrop:bg-transparent">
            <el-dialog-backdrop
                    class="fixed inset-0 bg-gray-900/50 transition-opacity data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in"></el-dialog-backdrop>
                    ...
                    <div class="bg-white relative px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex flex-col sm:items-start">
                            <div class="flex pb-4 justify-between w-full items-center">
                                ...
                                <button type="button" command="close" commandfor="invite-company-model"
                                        class="rounded-full p-2 text-gray-800 hover:bg-gray-200 transition-all duration-300 cursor-pointer absolute right-3 top-3">
                                    <svg class="size-[18px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                         fill="currentColor">
                                        <path d="M11.9997 10.5865L16.9495 5.63672L18.3637 7.05093L13.4139 12.0007L18.3637 16.9504L16.9495 18.3646L11.9997 13.4149L7.04996 18.3646L5.63574 16.9504L10.5855 12.0007L5.63574 7.05093L7.04996 5.63672L11.9997 10.5865Z">
                            ...
                            <form class="w-full" id="inviteCompanyForm">
                                @csrf
                                ...
                                <div class="flex flex-col gap-1 pb-4">
                                    ...
                                    <input type="text" value="" id="invitation_email"
                                           name="invitation_email"
                                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                           placeholder="{{ trans_message('company_email') }}">
    ...
    <!-- Update Company Admin Model -->
    <el-dialog>
        <dialog id="update-company-admin-model" aria-labelledby="dialog-title"
                class="fixed inset-0 size-auto max-h-none max-w-none overflow-y-auto bg-transparent backdrop:bg-transparent">
            <el-dialog-backdrop
                    class="fixed inset-0 bg-gray-900/50 transition-opacity data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in"></el-dialog-backdrop>
            ...
            <div tabindex="0"
                ...
                <el-dialog-panel
                    ...
                    <div class="bg-white relative px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex flex-col sm:items-start">
                            <div class="flex pb-4 justify-between w-full items-center">
                                <h3 id="update-person-dialog-title" class="text-base font-semibold text-gray-800">
                                    {{ trans_message('edit_contact_person') }}</h3>
                                <button type="button" command="close" commandfor="update-company-admin-model"
                                        class="rounded-full p-2 text-gray-800 hover:bg-gray-200 transition-all duration-300 cursor-pointer absolute right-3 top-3">
                                    <svg class="size-[18px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                         fill="currentColor">
                                        <path d="M11.9997 10.5865L16.9495 5.63672L18.3637 7.05093L13.4139 12.0007L18.3637 16.9504L16.9495 18.3646L11.9997 13.4149L7.04996 18.3646L5.63574 16.9504L10.5855 12.0007L5.63574 7.05093L7.04996 5.63672L11.9997 10.5865Z">
                            ...
                            <form class="w-full" id="editCompanyUserForm">
                                @csrf
                                ...
                                <div class="flex flex-col items-center">
                                    <div class="relative">
                                        ...
                                        <div id="user-preview-container"
                                             class="w-24 h-24 bg-gray-200 rounded-full flex items-center justify-center mb-2 overflow-hidden">
                                            <svg id="user-default-icon" class="w-12 h-12 text-gray-400"
                                                 xmlns="http://www.w3.org/2000/svg"
                                                 viewBox="0 0 24 24" fill="currentColor">
                                                <path d="M12 12C14.7614 12 17 9.76142 17 7C17 4.23858 14.7614 2 12 2C9.23858 2 7 4.23858 7 7C7 9.76142 9.23858 12 12 12Z"/>
                                                <path d="M12 14C7.58172 14 4 17.5817 4 22H20C20 17.5817 16.4183 14 12 14Z"/>
                                            </svg>
                                            <img id="user-preview-image" src="" alt="Preview"
                                                 class="hidden w-full h-full object-cover"/>
                                        </div>

                                        <!-- File Input (hidden) -->
                                        <input type="file" id="user-file-input" accept=".jpeg,.jpg,.png,.gif,.avif,.svg"
                                               class="hidden" name="image"/>
                                ...
                                <div class="flex flex-col gap-1 pb-4">
                                    ...
                                    <input type="text" value="" name="name"
                                           class="name w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                ...
                                <div class="flex flex-col gap-1 pb-4">
                                    ...
                                    <input type="email" value="" name="email"
                                           ...
                                           placeholder="{{ trans_message('email') }}" readonly>
            ...
            <div class="flex items-center gap-4 w-full md:w-auto">
                <div class="relative w-full md:w-auto">
                    <select id="filter-status"
                        ...
                        <option value="DEACTIVATE">{{ trans_message('deactivate') }}</option>
                    </select>
...
{{--                        <option value="">{{ trans_message('sort_by') }}</option>--}}
                ...
                <div class="relative w-full hidden lg:flex md:w-auto">
                    <button command="show-modal" commandfor="add-company-model"
                            class="btn-primary w-full appearance-none border border-[#e5e7eb] bg-gray-100 rounded-full px-4 py-2.5 text-gray-700 cursor-pointer transition-all duration-300"
                            onclick="resetForm();">
                        <span>{{ trans_message('add_company') }}</span>
                    </button>
                </div>
            </div>
        </div>
...
@section('script')
    <script>
        ...
        const userDefaultIcon = document.getElementById("user-default-icon");
        ...
        userFileInput.addEventListener("change", function () {
            ...
            if (file) {
                const reader = new FileReader();
                reader.onload = function (e) {
                    userPreviewImage.src = e.target.result;
                    userPreviewImage.classList.remove("hidden");
                    userDefaultIcon.classList.add("hidden");
                };
                reader.readAsDataURL(file);
            }
        });
        ...
        $(document).ready(function () {
            ...
            $("#addCompanyForm").validate({
                ...
                messages: {
                    company_name: {required: "{{ trans_message('company_name_required') }}"},
                },
                ...
                submitHandler: function (form, e) {
                    ...
                    $.ajax({
                        ...
                        cache: false,
                        beforeSend: function () {
                            $('#addCompanyBtn').attr('disabled', true).html(`
                        ...
                        complete: function () {
                            $('#addCompanyBtn').attr('disabled', false).html('{{ trans_message('save') }}');
                            companyList();
                        },
                    });
                }
            ...
            $("#editCompanyForm").validate({
                ...
                submitHandler: function (form, e) {
                    ...
                    $.ajax({
                        ...
                        cache: false,
                        beforeSend: function () {
                            $('#editCompanyBtn').attr('disabled', true).html(`
                                <svg class="animate-spin h-4 w-4 text-white mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
                        ...
                        complete: function () {
                            $('#editCompanyBtn').attr('disabled', false).html('{{ trans_message('save') }}');
                            companyList();
                        },
                    });
                }
            ...
            // Invite Company Admin Form
            $("#inviteCompanyForm").validate({
                rules: {
                ...
                submitHandler: function (form, e) {
                    ...
                    $.ajax({
                        ...
                        cache: false,
                        beforeSend: function () {
                            $('#inviteCompanyBtn').attr('disabled', true).html(`
                                <svg class="animate-spin h-4 w-4 text-white mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
                        ...
                        success: function (result) {
                            $('#invite-company-model')[0].close();
                            sendToast(result.message);
                            var company_id = $('#inviteCompanyForm input[name=company_id]').val();
                            companyAdminsList(company_id);
                            $("#inviteCompanyForm").trigger('reset');
                        },
                        error: function (xhr) {
                            let data = xhr.responseJSON;
                            if (data.hasOwnProperty('error')) {
                        ...
                        complete: function () {
                            $('#inviteCompanyBtn').attr('disabled', false).html('{{ trans_message('invite') }}');
                            companyList();
                        },
                    });
                }
            ...
            $("#editCompanyUserForm").validate({
                ...
                submitHandler: function (form, e) {
                    ...
                    $.ajax({
                        ...
                        cache: false,
                        beforeSend: function () {
                            $('#editCompanyUserBtn').attr('disabled', true).html(`
                                <svg class="animate-spin h-4 w-4 text-white mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
                        ...
                        success: function (result) {
                            $('#update-company-admin-model')[0].close();
                            sendToast(result.message);
                            var company_id = $('#inviteCompanyForm input[name=company_id]').val();
                            companyAdminsList(company_id);
                        },
                        ...
                        complete: function () {
                            $('#editCompanyUserBtn').attr('disabled', false).html('{{ trans_message('save') }}');
                        },
                    });
                }
            });
        ...
        function resendInviteCompanyUser(id, company_id, invitation_email, button) {
            $.ajax({
                url: "{{route('admin.company.users.re.invite')}}",
                method: "post",
                dataType: "json",
                data: {
                    id: id,
        ...
        function removeCompany(id, element) {
            Swal.fire({
                ...
                text: "{{ trans_message('remove_company_confirm') }}",
                icon: "warning",
                showCancelButton: !0,
        ...
        function getCompany(id, element) {
            $.ajax({
                ...
                success: function (data) {
                    resetForm();
                    $("#edit-company-model #editId").val(id);
                    $('#edit-company-model .editCompanyId').html(data.data.company_id);
                    $('#inviteCompanyForm input[name=company_id]').val(data.data.id);

                    $('#edit-company-model #editCompanyName').val(data.data.company_name);
                    $('#subdealerSelect').val(data.data.subdealer_id);
                    $('#edit-company-model .sr-only').removeAttr('checked', 'checked');
                    if (data.data.status == 'ACTIVE') {
                        $('#edit-company-model .sr-only').attr('checked', 'checked');
                    }
                    ...
                    if (data.data.image) {
                        ...
                        $("#edit-company-model #edit-preview-image").attr('src', data.data.image).show();
                    }

                    companyAdminsList(id);

                    $('#edit-company-model')[0].showModal();
                },
                error: function (xhr) {
                    let data = xhr.responseJSON;
                    if (data.hasOwnProperty('error')) {
        ...
        function companyAdminsList(company_id, element) {
            $.ajax({
                ...
                success: function (data) {
                    $(".company_admins_list").html(data.data.html);
                },
                error: function (xhr) {
                    let data = xhr.responseJSON;
                    if (data.hasOwnProperty('error')) {
        ...
        // Remove company user
        function removeCompanyUser(id, company_id, element) {
            $('#edit-company-model')[0].close();
            Swal.fire({
                title: "Are you sure?",
        ...
        function getCompanyUser(id, company_id, name, email, mobile_number, image, element) {

            $('#editCompanyUserForm input[name=id]').val(id);
            $('#editCompanyUserForm input[name=name]').val(name);
            $('#editCompanyUserForm input[name=email]').val(email);
            $('#editCompanyUserForm input[name=mobile_number]').val(mobile_number);

            // Set image if exists
            if (image) {
                $('#user-default-icon').addClass('hidden');
                $('#user-preview-image').removeClass('hidden');
                $("#editCompanyUserForm #user-preview-image").attr('src', image).show();
            }

            $('#update-company-admin-model')[0].showModal();
        }
